#!/usr/bin/env python3
"""S10-C3-C: 4-arm paired search NPS harness (candidate evaluation).

Evolves the C3-B protocol with a frozen BASELINE binary arm (C0) so
candidate measurements are same-session paired ratios immune to
microbench/machine drift. Original 3-arm C3-B protocol lives in
search_nps_c3b.py.
...
def main() -> int:
    ...
    if model_sha != MODEL_SHA:
        ...
        return 4
    # S10-C3-C Repair 1 (P2): hash the ACTUAL baseline binary and
    # fail closed against the expected frozen SHA.
    if not BASELINE_ENGINE.is_file():
        print(f"FATAL: baseline binary missing: {BASELINE_ENGINE}")
        return 4
    actual_baseline_sha = hashlib.sha256(
        BASELINE_ENGINE.read_bytes()).hexdigest()
    if actual_baseline_sha != BASELINE_ENGINE_SHA:
        print(f"FATAL: baseline binary SHA {actual_baseline_sha} != "
              f"expected {BASELINE_ENGINE_SHA}")
        return 4
