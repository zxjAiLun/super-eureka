...
fn default_search_does_not_collect_diagnostic_counters() {
    ...
    assert_eq!(stats.s75b_checking_edges, 0);
    assert_eq!(stats.s75b_check2_child_seen, 0);
    assert_eq!(stats.s75b_check2_at_parent_depth1, 0);
    assert_eq!(stats.s75b_check2_at_parent_depth2plus, 0);
    assert_eq!(stats.s75b_check2_budget2, 0);
    assert_eq!(stats.s75b_check2_budget1, 0);
    assert_eq!(stats.s75b_check2_budget0, 0);
    assert_eq!(stats.s75b_check2_followed_by_single_evasion, 0);
    assert_eq!(stats.s75b_single_evasion_followed_by_check2, 0);
    assert_eq!(stats.s75b_probe_calls, 0);
    assert_eq!(stats.s75b_probe_pseudo_moves, 0);
    assert_eq!(stats.s75b_probe_legality_tests, 0);
    assert_eq!(stats.s75b_probe_claim_skipped, 0);
